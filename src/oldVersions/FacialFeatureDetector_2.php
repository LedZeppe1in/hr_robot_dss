    ...
    public function detectTrends($sourceFaceData1, $trendLength)
    {
        if ($sourceFaceData1 != null)
            foreach ($sourceFaceData1 as $k => $v)
                if((strpos($k,'frame') === false) && (strpos($k,'MASK_NAMES') === false) && (strpos($k,'statistics') === false)){
                if ($v != null) {
                    foreach ($v as $k1 => $v1) {
...
//                                                        'yes', $eyeStartClosingFrame, ($i + $eyeClosedFrame - $eyeStartClosingFrame));
    ...
    public function detectFeatureStatistics($sourceFaceData1,$coefs_)
    {
        //на вход нужно подать время
            //1. время видео на вопрос
            // 2. время озвучивания вопроса


        $sourceFaceData1["feature_statistics"]=array();
    }

    public function detectSummarizedFeatureStatistics($sourceFaceData1,$coefs_)
    {
...
//                $this->saveXY2($FaceData,'m1.json');
        ...
        $detectedFeaturesWithTrends = $this->detectFeatureStatistics($detectedFeaturesWithTrends,$coefs);
